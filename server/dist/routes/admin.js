...
router.get('/users', async (req, res) => {
    ...
    if (!isSystemAdmin) {
        if (classId) {
            ...
            let hasAccess = false;
            if (classAdminClassIds.includes(classId))
                hasAccess = true;
            else {
    ...
    const result = users.map(u => {
        ...
        if (allRoles.includes('system_admin'))
            primaryRole = 'system_admin';
        else if (allRoles.includes('school_admin'))
            primaryRole = 'school_admin';
        else if (allRoles.includes('class_admin'))
            primaryRole = 'class_admin';
        return {
            ...u,
    ...
    if (!isSystemAdmin) {
        ...
        if (classAdminClassIds.includes(scope_id)) {
            ...
            if (cls && schoolAdminSchoolIds.includes(cls.school_id)) {
                ...
                if (['student', 'class_admin'].includes(role))
                    hasAccess = true;
            }
        }
        if (!hasAccess)
            return res.status(403).json({ error: 'Forbidden: Insufficient permissions for this assignment' });
    }
    if (role === 'student') {
        // Add to class_memberships (without role column)
        if (!scope_id)
            return res.status(400).json({ error: 'Class ID required for student role' });
...
router.delete('/users/:id/roles', async (req, res) => {
    ...
    if (!isSystemAdmin) {
        if (!scope_id)
            return res.status(403).json({ error: 'Forbidden: Scope ID required' });
        const schoolAdminSchoolIds = userRoles.filter(r => r.role === 'school_admin' && r.scope_type === 'school').map(r => r.scope_id);
        const classAdminClassIds = userRoles.filter(r => r.role === 'class_admin' && r.scope_type === 'class').map(r => r.scope_id);
        let hasAccess = false;
        if (classAdminClassIds.includes(scope_id)) {
    ...
    if (role === 'student') {
        if (!scope_id)
            return res.status(400).json({ error: 'Class ID required to remove student membership' });
        const { error } = await supabase
            .from('class_memberships')
            .delete()
            .eq('user_id', userId)
        ...
        if (error)
            return res.status(500).json({ error: 'Failed to remove student membership' });
    }
    else {
...
router.put('/schools/:id', requireSystemAdmin, async (req, res) => {
    const id = req.params.id;
    const schema = z.object({ name: z.string().min(1) });
    const parsed = schema.safeParse(req.body);
    if (!parsed.success)
...
router.delete('/schools/:id', requireSystemAdmin, async (req, res) => {
    const id = req.params.id;
