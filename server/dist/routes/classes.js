...
router.post('/join-requests', async (req, res) => {
    ...
    const { data: dupPending } = await supabase
        ...
        .maybeSingle();
    if (dupPending)
        return res.status(409).json({ error: 'DuplicatePending' });
...
router.get('/:class_id/join-requests', async (req, res) => {
    const userId = getUserId(req);
    if (!userId)
        return res.status(401).json({ error: 'Unauthorized' });
...
router.post('/:class_id/join-requests/:request_id/approve', async (req, res) => {
    const userId = getUserId(req);
    if (!userId)
        return res.status(401).json({ error: 'Unauthorized' });
    const classId = req.params.class_id;
    ...
    const { data: jr, error: jErr } = await supabase
        ...
        .eq('id', requestId)
        .maybeSingle();
    if (jErr || !jr || jr.class_id !== classId)
    ...
    if (jr.status !== 'pending') {
        ...
        if (upErr)
            return res.status(500).json({ error: 'Failed to ensure membership' });
...
router.post('/:class_id/join-requests/:request_id/reject', async (req, res) => {
    const userId = getUserId(req);
    if (!userId)
        return res.status(401).json({ error: 'Unauthorized' });
    const classId = req.params.class_id;
...
router.get('/:class_id/members', async (req, res) => {
    ...
    let allow = admin;
    if (!allow)
        allow = await isMemberOfClass(userId, classId);
    if (!allow)
        return res.status(403).json({ error: 'Forbidden' });
...
router.get('/:class_id/selected-plan', async (req, res) => {
    const userId = getUserId(req);
    if (!userId)
        return res.status(401).json({ error: 'Unauthorized' });
    const classId = req.params.class_id;
    ...
    const { data, error } = await supabase
        ...
        .maybeSingle();
    if (error)
        return res.status(500).json({ error: 'Failed to get selected plan' });
    if (!data)
        return res.json({ selected_plan: null });
    res.json({ selected_plan: data });
});
router.put('/:class_id/selected-plan', async (req, res) => {
    const userId = getUserId(req);
    if (!userId)
        return res.status(401).json({ error: 'Unauthorized' });
    const classId = req.params.class_id;
    ...
    const { data: plan, error: pErr } = await supabase
        ...
        .single();
    if (pErr || !plan)
        return res.status(404).json({ error: 'Optional plan not found' });
    const { data, error } = await supabase
        .from('selected_plans')
        .upsert({
        class_id: classId,
        ...
        .single();
    if (error)
        return res.status(500).json({ error: 'Failed to set selected plan' });
    res.json({ selected_plan: data });
});
