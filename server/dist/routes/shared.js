...
router.get('/:token', async (req, res) => {
    ...
    if (share.scope === 'blocks_only') {
        const sanitized = (blocks || []).map(b => ({ start_at: b.start_at, end_at: b.end_at }));
        return res.json({ share: { scope: 'blocks_only', expires_at: share.expires_at }, blocks: sanitized });
    }
    ...
    return res.json({ share: { scope: 'full', expires_at: share.expires_at }, tasks, blocks: outBlocks });
});
router.post('/:token/copy', async (req, res) => {
    ...
    if (share.scope === 'blocks_only') {
        ...
        if (payload.length === 0)
            return res.json({ created_blocks: 0 });
        const { error: insErr } = await supabase.from('time_blocks').insert(payload);
        if (insErr)
            return res.status(500).json({ error: 'Failed to copy blocks' });
    ...
    if (taskIds.length > 0) {
        for (const tid of taskIds) {
            ...
            const { data: newT, error: iErr } = await supabase
                ...
                .single();
            if (iErr || !newT)
                return res.status(500).json({ error: 'Failed to copy task' });
