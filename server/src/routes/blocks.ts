...
async function updateTaskSchedulingStatus(taskId: string) {
...
router.post('/', async (req: Request, res: Response) => {
  ...
  if (new Date(endIso) <= new Date(startIso)) return res.status(400).json({ error: 'end_at must be after start_at' });
...
router.patch('/:id', async (req: Request, res: Response) => {
  ...
  const { data: before, error: bErr } = await supabase
    ...
    .eq('user_id', userId)
    .single();
  if (bErr || !before) return res.status(404).json({ error: 'Not found' });
  ...
  const { error: uErr } = await supabase
    ...
    .eq('id', id)
    .eq('user_id', userId);
  if (uErr) return res.status(500).json({ error: 'Failed to update block' });

  if (before.task_id !== newTaskId) {
    if (before.task_id) await updateTaskSchedulingStatus(before.task_id);
    if (newTaskId) await updateTaskSchedulingStatus(newTaskId);
  }
  res.json({ message: 'OK' });
});
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  const { error } = await supabase
    ...
    .eq('id', id)
    .eq('user_id', userId);
  if (error) return res.status(500).json({ error: 'Failed to delete block' });
  if (before.task_id) await updateTaskSchedulingStatus(before.task_id);
  res.json({ message: 'OK' });
});
