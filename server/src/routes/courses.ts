...
router.post('/import', upload.single('file'), async (req: Request, res: Response) => {
  ...
  for (const k of required) {
    if (!headers.includes(k)) return res.status(400).json({ error: `Missing column: ${k}` });
  }

  // Build unique courses by (code, term)
  const uniqKeys = new Map<string, { code: string; name: string; term: string }>();
  for (const r of records) {
    const code = String(r.course_code || '').trim();
    const name = String(r.course_name || '').trim();
    const term = String(r.term || '').trim();
    const key = `${code}||${term}`;
    if (!uniqKeys.has(key)) uniqKeys.set(key, { code, name, term });
  }
  ...
  for (const [key, { code, name, term }] of uniqKeys.entries()) {
    const { data: existing, error: selErr } = await supabase
      ...
      .eq('term', term)
      .maybeSingle();
    if (selErr) return res.status(500).json({ error: 'DB error (select course)' });
    if (existing) {
      ...
      const { data: ins, error: insErr } = await supabase
        ...
        .select('id')
        .single();
      if (insErr || !ins) return res.status(500).json({ error: 'DB error (insert course)' });
  ...
  for (const r of records) {
    ...
    if (!course_id) continue; // should not happen
  ...
  if (sessions.length > 0) {
    ...
    if (sessErr) return res.status(500).json({ error: 'DB error (insert sessions)' });
  }
...
router.get('/', async (req: Request, res: Response) => {
  ...
  if (error) return res.status(500).json({ error: 'Failed to list courses' });
  res.json({ items: data });
});
