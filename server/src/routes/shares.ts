...
router.post('/', async (req: Request, res: Response) => {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) return res.status(500).json({ error: 'Failed to create share' });
...
router.get('/', async (req: Request, res: Response) => {
  ...
  if (error) return res.status(500).json({ error: 'Failed to list shares' });
  res.json({ items: data });
});
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  const { error } = await supabase
    ...
    .eq('id', id)
    .eq('owner_user_id', userId);
  if (error) return res.status(500).json({ error: 'Failed to delete share' });
  res.json({ message: 'OK' });
});
