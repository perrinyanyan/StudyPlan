...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  if (fetchErr || !tagRow) return res.status(404).json({ error: 'Tag not found' });
  ...
  if (checkErr) return res.status(500).json({ error: 'Failed to check tag usage' });

  // Filter out any null tasks (shouldn't happen with inner join but safety first)
  const validTasks = usedTasks?.map((ut: any) => ut.tasks).filter(Boolean) || [];

  if (validTasks.length > 0 && !force) {
    return res.status(409).json({
      error: 'Tag is in use',
      tasks: validTasks.map((t: any) => ({ title: t.title, due_at: t.due_at }))
    });
  }
  ...
  if (linkErr) return res.status(500).json({ error: 'Failed to unlink tag' });
