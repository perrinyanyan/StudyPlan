...
router.get('/', async (req: Request, res: Response) => {
  ...
  if (error) return res.status(500).json({ error: 'Failed to list task types' });
  res.json({ items: data });
});
...
router.post('/', async (req: Request, res: Response) => {
  ...
  const { data, error } = await supabase
    ...
    .select('id')
    .single();
  if (error) return res.status(500).json({ error: 'Failed to create task type' });
...
router.patch('/:id', async (req: Request, res: Response) => {
  ...
  if (oldErr || !oldType) return res.status(404).json({ error: 'Task type not found' });
  ...
  if (error) return res.status(500).json({ error: 'Failed to update task type' });

  // Cascade update to tasks if name or color changed
  if (name || color) {
    const taskUpdate: any = {};
    if (name) taskUpdate.type = name;
    if (color) taskUpdate.color = color;
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  if (fetchErr || !typeRow) return res.status(404).json({ error: 'Task type not found' });
  ...
  if (checkErr) return res.status(500).json({ error: 'Failed to check type usage' });
  ...
  if (force && usedTasks && usedTasks.length > 0) {
    ...
    if (updateErr) return res.status(500).json({ error: 'Failed to decouple tasks' });
  }
  ...
  if (error) return res.status(500).json({ error: 'Failed to delete task type' });
